...
data ZOO.TRANSACTION_DETAILS;
	...
	keep detail_id transaction_id ticket_type_id quantity;
run; 

data ZOO.TRANSACTION_DETAILS;
	set ZOO.TRANSACTION_DETAILS;
	detail_id=_N_;
run;
/*********************/
...
data ZOO.RESPONSIBLE_STAFF;
	...
	keep pair_id employee_id object_id;
run;
...
data ZOO.SUPPLIES_DETAILS;
	length detail_id 8 supply_id 8 food_id 4 quantity 5 unit $3 price 4;
	...
	keep detail_id supply_id food_id quantity unit price;

run;

data ZOO.SUPPLIES_DETAILS;
	set ZOO.SUPPLIES_DETAILS;
	detail_id=_N_;
run;
...
ALTER TABLE ZOO.TICKET_TYPES ADD CONSTRAINT PK_TICKET_TYPES PRIMARY KEY (guid);
...
ALTER TABLE ZOO.TRANSACTION_DETAILS  ADD CONSTRAINT PK_TRANSACTION_DETAILS PRIMARY KEY (detail_id);
ALTER TABLE ZOO.OBJECTS  ADD CONSTRAINT PK_OBJECTS PRIMARY KEY (object_id);
ALTER TABLE ZOO.RESPONSIBLE_STAFF ADD CONSTRAINT PK_RESPONSIBLE_STAFF PRIMARY KEY (pair_id); 
...
ALTER TABLE ZOO.SUPPLIES_DETAILS ADD CONSTRAINT PK_SUPPLIES_DETAILS PRIMARY KEY (detail_id);
...
PROC SQL;
ALTER TABLE ZOO.EMPLOYEES ADD CONSTRAINT FK_EMPLOYEES_CONTRACT_TYPES FOREIGN KEY(contract_type)
REFERENCES ZOO.CONTRACT_TYPES ON DELETE RESTRICT ON UPDATE SET NULL;

ALTER TABLE ZOO.EMPLOYEES ADD CONSTRAINT FK_EMPLOYEES_POSITIONS FOREIGN KEY(position_code)
...
PROC SQL;
ALTER TABLE ZOO.TRANSACTION_DETAILS ADD CONSTRAINT FK_TRANDETAILS_TICKETTYPES FOREIGN KEY(ticket_type_id)
REFERENCES ZOO.TICKET_TYPES ON DELETE RESTRICT ON UPDATE SET NULL;
